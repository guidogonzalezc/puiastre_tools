...
class EyeModule():
    ...
    def create_chain(self):
        ...
        for curve in self.guides[1:]:
            ...
            cmds.select(rebuild_curve, r=True)
        ...
        for i, curves in enumerate(bezier_curves):
            name = curve.replace("_CRV", f"Blink")
        ...
        for index_main, curve in enumerate(bezier_curves):
            ...
            self.eyelid_rotation_matrix = cmds.getAttr(f"{self.eyelid_rotation}.outputMatrix")
            cmds.setAttr(f"{four_by_four_mid}.in30", cmds.pointPosition(f"{curve}.cv[{6}]")[0])
            cmds.setAttr(f"{four_by_four_mid}.in31", cmds.pointPosition(f"{curve}.cv[{6}]")[1])
            cmds.setAttr(f"{four_by_four_mid}.in32", cmds.pointPosition(f"{curve}.cv[{6}]")[2])

            four_by_four_mid_blink = cmds.createNode("fourByFourMatrix", name=curve.replace("_CRV", f"MiddleBlink_F4X"), ss=True)
            cmds.connectAttr(f"{curve}.editPoints[6].xValueEp", f"{four_by_four_mid_blink}.in30", force=True)
            cmds.connectAttr(f"{curve}.editPoints[6].yValueEp", f"{four_by_four_mid_blink}.in31", force=True)
            cmds.connectAttr(f"{curve}.editPoints[6].zValueEp", f"{four_by_four_mid_blink}.in32", force=True)
            ...
            for i, cv_index in enumerate([3, 6, 9]):
                ...
                cmds.connectAttr(f"{controller_grp[0]}.worldInverseMatrix[0]", f"{multmatrix}.matrixIn[3]", force=True)
                ...
                for j, value in enumerate([-1, 1]):
                    ...
                    ctl, controller_grp = controller_creator(
                    name=f"{curve.replace('_CRV', f'{name}Tangets0{j+1}')}",
                    suffixes=["GRP", "OFF","ANM"],
                    ...
                    cmds.setAttr(f"{four_by_four_tangent}.in30", cmds.pointPosition(f"{curve}.cv[{cv_index}]")[0])
                    cmds.setAttr(f"{four_by_four_tangent}.in31", cmds.pointPosition(f"{curve}.cv[{cv_index}]")[1])
                    cmds.setAttr(f"{four_by_four_tangent}.in32", cmds.pointPosition(f"{curve}.cv[{cv_index}]")[2])
