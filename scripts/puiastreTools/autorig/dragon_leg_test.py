...
class LimbModule(object):
    ...
    def make(self):
        ...
        blend_matrix = cmds.createNode("blendMatrix", name=f"{self.side}_{self.module_name}Guide04_BLM", ss=True)
        cmds.connectAttr(f"{aim_matrix_guides[-1]}.outputMatrix", f"{blend_matrix}.inputMatrix", force=True)
        cmds.connectAttr(self.leg_guides[0], f"{blend_matrix}.target[0].targetMatrix", force=True)
        ...
        self.leg_guide_matrix = blend_matrix
    ...
    def ik_rig(self):
        ...
        self.upper_ik_ctl, self.upper_ik_ctl_grp = controller_creator(
            name=f"{self.side}_{self.module_name}UpperIk",
            suffixes=["GRP", "ANM"],
            lock=["visibility"],
            ro=True,
            parent=self.ik_controllers
        ...
        upper_ctl_multmatrix = cmds.createNode("multMatrix", name=f"{self.side}_{self.module_name}UpperIkOffset_MMX", ss=True)
        cmds.connectAttr(f"{self.guides_matrix[-1]}.outputMatrix", f"{upper_ctl_multmatrix}.matrixIn[0]")
        cmds.connectAttr(self.leg_guides[0].replace("worldMatrix[0]", "worldInverseMatrix[0]"), f"{upper_ctl_multmatrix}.matrixIn[1]")
        cmds.connectAttr(f"{self.hand_ik_ctl}.worldMatrix[0]", f"{upper_ctl_multmatrix}.matrixIn[2]")
        cmds.connectAttr(f"{upper_ctl_multmatrix}.matrixSum", f"{self.upper_ik_ctl_grp[0]}.offsetParentMatrix")
        ...
        self.ikHandleManager = f"{self.upper_ik_ctl}.worldMatrix[0]"
